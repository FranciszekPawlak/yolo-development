...
export function FixedCostsTable({ data }: FixedCostsTableProps) {
	...
	return (
		<Card className="border border-zinc-800 bg-zinc-900/60 p-4 sm:p-5">
			<h3 className="mb-3 flex items-center gap-2 text-xs font-semibold text-zinc-200 sm:mb-4 sm:text-sm">
				<span>📌</span> Koszty stałe
			</h3>
			<div className="-mx-4 overflow-x-auto px-4 sm:-mx-5 sm:px-5">
				<table className="w-full min-w-[360px] text-xs sm:text-sm">
					...
					<tbody className="divide-y divide-zinc-800/60">
						{data.map((row) => (
							<tr key={row.category}>
								<td className="py-2 pr-3 text-zinc-200 sm:py-2.5 sm:pr-4">
									{row.label}
								</td>
